package code
;
import xyz.jordanplayz158.ptd1.GfxMovieClip;
import xyz.jordanplayz158.ptd1.SamURLVariables;
import openfl.net.*;
import openfl.display.*;
import openfl.*;
import compat.*;
class Class_576 extends Class_194 {


    public function new(param1:Poke) {
        super(param1);
        var_7 = "Stored Power";
    }

    @:allow(code) override function get_Move_Power():Int {
        return 20;
    }

    @:allow(code) override function do_Attack(param1:Poke) {
        var _loc2_ = this.get_Move_Power();
        var _loc3_ = 1;
        if (var_2.check_Effects(Class_828) != null) {
            _loc3_++;
        }
        if (var_2.check_Effects(Class_795) != null) {
            _loc3_++;
        }
        if (var_2.check_Effects(Class_806) != null) {
            _loc3_++;
        }
        if (var_2.check_Effects(Class_805) != null) {
            _loc3_++;
        }
        if (var_2.check_Effects(Class_808) != null) {
            _loc3_++;
        }
        if (var_2.check_Effects(Class_803) != null) {
            _loc3_++;
        }
        if (var_2.check_Effects(Class_801) != null) {
            _loc3_++;
        }
        if (_loc3_ > 1) {
            _loc3_ += 3;
        }
        var _loc4_ = method_3(param1, _loc2_ * _loc3_);
        var _loc5_:Class_16;
        (_loc5_ = new Class_312(var_2, param1, _loc4_)).var_12 = var_12;
        var_2.add_Effect(_loc5_);
        var_2.add_Effect(new Class_205(var_2, 16711680, 0.7, Attack.ATTACK_TIME));
    }

    @:allow(code) override function get_My_Attack_Animation():Class_16 {
        return new Class_16(var_2);
    }

    @:allow(code) override function get_Target_Hit_Animation(param1:Poke):Class_16 {
        return null;
    }

    @:allow(code) override function get_Move_Type():Int {
        return Class_13.const_14;
    }
}

